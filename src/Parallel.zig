//! Bounded, positional file encoder for Bao's post-order outboard format.
...
fn encodeSegment(work: *Work, k: usize) !void {
    ...
    var read_buf: [Bao.READ_BUF_SIZE]u8 align(8) = undefined;
    ...
    const root = try Bao.encodeSubtree(
        &reader.interface,
        seg.startCounter(),
...
pub fn encodeFile(
    ...
    const n_chunks = if (content_length == 0) 0 else (content_length - 1) / Bao.chunk_length + 1;
    return encodeWithSplit(io, allocator, input, content_length, output, workers, Bao.subtreeSplit(n_chunks, @intCast(workers)));
}
...
fn encodeWithSplit(
    ...
    return Bao.combineSubtreesPositional(split, content_length, roots, io, output);
}
...
fn compareToSequential(io: std.Io, tmp: std.Io.Dir, content: []const u8, workers: usize, seg_chunks: u64) !void {
    ...
    const n_chunks: u64 = if (content.len == 0) 0 else (content.len - 1) / Bao.chunk_length + 1;
    const split = Bao.Split{ .seg_chunks = if (n_chunks == 0) 0 else seg_chunks, .total_chunks = n_chunks };
    ...
    const expected = try Bao.encodeReader(&ref_reader, content.len, &ref_writer);
...
test "production split boundaries, invalid workers, and truncated input" {
    ...
    var empty_expected: Bao.Hash = undefined;
    ...
    var expected: Bao.Hash = undefined;
    ...
    try testing.expectError(error.UnexpectedEof, encodeFile(io, testing.allocator, input, Bao.chunk_length + 1, output, 2));
}
